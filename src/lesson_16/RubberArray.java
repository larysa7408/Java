...
public class RubberArray {
    ...
    int[] array;//1/МАССИВ интов который называется array. чтобы работа была более комфортной и универсальной

    int cursor;// 4/ текущий ИНДЕКС массива которому мы будем присваивать значение по умолчанию 0// чтобы записать наш массив нам нужен индекс
//cursor это длина внутреннего  значения значиемого массива
    ...
    void add(int number) {// 3/По нашей задумке мы должны иметь  МЕТОД который будет называться add и принимать значение (int number)
...
//динамическое изменение размера массива
    void expandArray() {// Создали метод
...
//Вывод в консоль для значения массива
    public String toString() { // выводим МЕТОД который выведет в консоль, для этого нужно перебрать МАССИВ элементы которые мы будем добавлять
        if (cursor == 0) return "[]"; // поправляем метод и добавляем если cursor == 0 у нас в метод ничего не записанно тоесть у нас еще ничего не записанно то будет "[]"
        //
